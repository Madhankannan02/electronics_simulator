...
export default function Diode() {
  ...
  const [inputVoltage, setInputVoltage] = useState<number>(0);

  // Calculate standard silicon rectifier diode behavior (e.g., 1N4007)
  const forwardDrop = 0.7; // Standard silicon P-N junction potential barrier
  let outputVoltage = 0;
  let diodeCurrent = 0; // in mA
  let mode: 'blocking' | 'forward' = 'blocking';

  if (inputVoltage > 0) {
    if (inputVoltage < forwardDrop) {
      // Sub-threshold conduction region
      outputVoltage = inputVoltage;
      diodeCurrent = (Math.pow(inputVoltage / forwardDrop, 4)) * 0.5; // very low leakage
      mode = 'blocking';
    } else {
      // Fully forward conducting
      outputVoltage = forwardDrop;
      // Current governed by typical dynamic forward resistance (simulated)
      diodeCurrent = (inputVoltage - forwardDrop) * 40; // 40mA per volt beyond junction drop
      mode = 'forward';
    }
  } else {
    // Reverse biased - blocks completely (leakage is negligible nano-Amps)
    outputVoltage = inputVoltage;
    diodeCurrent = 0.0001; // essentially zero
    mode = 'blocking';
  }

  // Display-friendly values
  const formattedVoltage = outputVoltage.toFixed(2);
  const formattedCurrent = diodeCurrent >= 1000
      ...
      <div className="w-full bg-slate-50 rounded-xl p-3 border border-slate-100 flex flex-col gap-1.5 mt-auto">
        ...
        <div className="flex flex-col">
          ...
          <div className="flex justify-between text-[7px] text-gray-400 mt-1">
            <span>-10V (Reverse Block)</span>
            <button onClick={() => setInputVoltage(0)} className="text-[7.5px] text-gray-400 flex items-center gap-0.5 hover:text-slate-650 transition-colors">
              <RotateCcw className="w-1.5 h-1.5" /> Reset (0V)
            </button>
            <span>+10V (Forward Bias)</span>
          </div>
        </div>
